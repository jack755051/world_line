...
- **系統色（介面底色/文字/灰階/主色/按鈕/狀態色）已定案**（2026-08-29 拍板）於 `app/src/app/core/design/design-tokens.scss`，全部來自 dataviz 技能驗證過的預設色盤，不是憑感覺挑的。跟政權識別色是分開的兩套：政權識別色可用分類色盤第 2-8 格（7 色，仍遠高於四色定理需要的下限），**第 1 格（藍）保留給 UI 主色專用**（按鈕/連結/焦點框），避免疆域填色跟互動元件顏色混淆。按鈕底色**用 dataviz 技能的 `contrast()` 工具實測而非用經驗猜**：分類色第 1 格（`#2a78d6`）配白字只有 4.42:1，未達 WCAG AA 一般文字門檻（4.5:1），所以按鈕預設底色改用深一階的主色（`#256abf`，配白字 5.39:1）；連結文字同理不用分類色原始值，改用同一個深一階版本（對面板底色 5.26:1）。灰階沿用既有已定案的幾個中性色點位，統一編號成一條 ramp，不是另外新調的顏色
