...
| [ ] | 2.16 | 雙語內容 schema（憲法 R4） | 新增**單一通用表** `content_translations`（`entity_type`/`entity_id`/`field_name`/`locale`/`translated_text`，見 PRD §6「多語言內容設計」）：EF Core entity/configuration + migration。範圍限中立事實內容（`regimes.self_name`、`regime_aliases.alias_name`、`historical_events.name`、`lineage_presets.preset_name`/`description`、`historical_event_controversies.topic`/`neutral_description`）——`historical_event_perspectives`、`viewpoints` 不進範圍。`historical_events.sections` JSONB 要不要連帶翻譯要先拍板再動工。純新增表，不影響現有資料 | §6、憲法 R4 | 1 個 |
