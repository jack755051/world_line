...
| [x] | 2.5 | 政權寫入端點 | `POST /api/v1/regimes`（I2 校驗自稱名稱必填）、`PATCH /api/v1/regimes/:id`（呼叫 2.1 驗證器擋非法轉換） | §7 | 1 個 |

> **2026-08-31 完成**：`api/Controllers/RegimesController.cs` 新增 `Create`/`Update`。
> **分裂下限問題已回報討論**——使用者確認**不在寫入端點檢查**：子政權是逐筆個別 POST
> 建立的，建立第一筆的當下不可能同步驗證「同一個 `predecessorRegimeId`+`split` 底下
> 總共會有幾筆」，這條數量完整性檢查交給 2.15 的 integration test 或人工檢視。
> **POST**：新建政權一律從 `active` 起步（憲法 §4 合法轉換全部從 active 出發），不接受
> `status` 欄位；呼叫 2.1 的 `ValidateOriginLinkage`（純函式一致性檢查）後，另外補兩條
> 跨政權 referential 檢查——`predecessorRegimeId` 必須存在（`PREDECESSOR_REGIME_NOT_
> FOUND`）、且不能已經是 `conquered`（`PREDECESSOR_ALREADY_CONQUERED`，任務描述給的
> 具體例子：被滅亡是暴力終結，跟分裂/禪讓出新政權不相容）。**PATCH**：呼叫 2.1 的
> `ValidateStatusTransition` 擋非法轉換（`INVALID_STATUS_TRANSITION`），另外驗證
> `destroyedByRegimeId` 跟「是否轉成 conquered」必須一致（見 `SeedData.cs` 既有慣例：
> 只有 conquered 才會設這個欄位）——轉 conquered 缺 `destroyedByRegimeId` 擋
> `DESTROYED_BY_REGIME_REQUIRED`、指到不存在的政權擋 `DESTROYED_BY_REGIME_NOT_
> FOUND`；非 conquered 卻帶了 `destroyedByRegimeId` 擋
> `DESTROYED_BY_REGIME_ONLY_FOR_CONQUERED`。**這是 API 第一個 PATCH 端點**，新增
> `ApiMessageCodes.UpdateSuccess`（`UPDATE_SUCCESS`），沿用 task 2.0 命名慣例（成功
> 代碼對應 HTTP 動詞語意）第一次真正套用到 PATCH。已用真實容器 curl 驗證 11 種案例
> （POST 缺 key/缺 selfName/獨立建國成功/origin 欄位不一致/predecessor 不存在/
> predecessor 已 conquered/合法禪讓成功；PATCH 缺 key/regime 不存在/非法轉換/轉
> conquered 缺 destroyedByRegimeId/destroyedByRegimeId 不存在/合法轉 conquered/非
> conquered 帶 destroyedByRegimeId）全數符合預期，OpenAPI 正確收錄兩個新端點，測試用
...
> 測試，延續既有慣例
| [x] | 2.9b | 地名雙軌查詢端點 | `GET /api/v1/place-names?year={y}`（依年份查當時使用中的地名，`valid_period` 區間匹配）、`GET /api/v1/place-names/:id`；回傳含 `historical_name`/`modern_name`（可為 NULL）雙欄位，不做寫入端點（seed 已覆蓋首都示範，正式匯入前的來源治理見 `docs/data-governance.md`） | 憲法 §6、§7 | 1 個 |

> **2026-08-31 完成**：`api/Controllers/PlaceNamesController.cs`。`year` 比照
> `reign-eras`（同樣是「依年份查 X」語意）拍板成必填，不是選填過濾條件，缺漏回 400
> `YEAR_REQUIRED`（重用既有代碼）。**`place_names.valid_period` 整欄可為 `NULL`
> 的處理**——這跟 `reign_eras.end_year` 為 `NULL`（尚未確定結束年，仍在使用中）不是
> 同一種語意：這裡代表「這個名稱的使用期間完全沒考證清楚」，兩種情況都算沒有明確
> 排除查詢年份，所以拍板成一律納入查詢結果（`p.ValidPeriod == null || p.ValidPeriod.
> Value.Contains(year)`），不是預設排除；已用真實容器 curl 確認 `NpgsqlRange<int>?`
> 這個 EF 型別組合能正確轉譯成 SQL（第一次嘗試就正確運作，沒有落坑）。**沒有寫入
> 端點**（任務範圍本來就排除）——seed 已覆蓋 4 筆首都示範（雒陽/洛陽/成都/建業），
> 正式匯入前的來源治理見 `docs/data-governance.md`。已用真實容器 curl 驗證：
> `year=100` 只回雒陽（25-220 區間內）、`year=250` 正確回三筆（洛陽/成都/建業，含
> 半開區間邊界判斷）、缺 `year` 回 400、`GET /place-names/:id` 成功／不存在（404
...
> `HistoricalEventPerspective`），這次只補控制器層——`api/Controllers/
> ObserverCategoriesController.cs`（唯讀，`GET /observer-categories`）、
> `api/Controllers/EventPerspectivesController.cs`（`GET`/`POST /events/:id/
> perspectives`）。**§12 TODO 的 JSON schema 這次拍板**：不是憑空另外設計一套，而是
> 沿用既有種子資料（赤壁之戰三筆視角）已經在用的形狀當「最小 citation 欄位」定案——
> `primary_sources` 是陣列，每筆 `{title, author, year?}`；`claimed_casualties` 是
> 物件，鍵名依史料實際記載彈性增減（例如 `{own_loss, enemy_loss}`），不強制固定欄位，
> 因為這不是引用清單、是「這個視角主張的一個數字/敘述」。跟 task 2.10 的 `sections`
> 同一個既有慣例：不寫程式碼層級的 JSON Schema 驗證，只在文件跟種子資料裡建立慣例。
> 已用真實容器 curl 驗證：`GET /observer-categories` 正確回一筆（後世史學界）、
> `GET /events/event-chibi-208/perspectives` 正確回三筆（後世史學界考據＋孫劉聯軍
> 視角＋東吳視角，`primarySources` 正確解析成巢狀 JSON 不是雙重編碼字串）、
> `POST .../perspectives` 缺兩個 party 正確回 400 `PERSPECTIVE_PARTY_REQUIRED`、
> 帶 `observerCategoryId` 正確 201、缺 `X-API-Key` 正確 401；測試用的資料事後從
> `app_postgres` 手動刪除，不留在種子資料裡（延續 task 2.10 起的既有慣例）。沒有另外
> 寫 xUnit 測試，同專案從 2.0 開始的慣例。
| [x] | 2.13 | 事件爭議點 | **寫入端點實作前先定義 `viewpoints` 的 JSON schema 與最小 citation 欄位（§12 TODO，可與 2.12 併案決定）**；`GET /api/v1/events/:id/controversies`、`POST .../controversies`。**依賴 2.16**：僅 `topic`/`neutral_description` 在翻譯範圍內（中立敘述），`viewpoints`（誰主張什麼）不翻譯，同 2.4 支援 `?locale=` | §6 notes §十.2、§12 TODO | 1 個 |

> **2026-08-31 完成**（跟 2.12 併案拍板 JSON schema，同一次對話動工）：
> `api/Controllers/EventControversiesController.cs`——`viewpoints` schema 沿用既有
> 三筆爭議點種子資料（曹操兵力/正統性歸屬/怛羅斯造紙術傳播）已經在用的形狀：陣列，
> 每筆 `{stance, source}`，`stance`+`source` 同時扮演「主張」跟「最小 citation」，
> 不疊加結構化引用清單，一個爭議點可以有兩個以上立場（正統性爭議就是三方）。
> `?locale=` 沿用 `EventsController.ApplyLocaleAsync()` 同一個模式（`topic`/
> `neutralDescription` 查 `historical_event_controversy_translations`，`viewpoints`
> 不受影響）。已用真實容器 curl 驗證：`GET .../controversies` 無 locale 正確回中文、
> `?locale=en` 正確回英文（`viewpoints` 保持中文原文不受影響）、`POST` 成功建立、
> 缺事件的 404 `EVENT_NOT_FOUND`；測試資料已清除。docs/api.md 這次沒有同步更新——
> 該文件從 2026-08-27 之後就沒有跟著任務進度更新（連已完成的 2.3 reign-eras 都還
> 標「尚未實作」），是既有落差，不是這次改動造成的，不在這個任務範圍內處理。
...
> 魏那兩筆既有爭議點資料狀態一致
>
> **2026-08-31 追加調整：選單改成依拉桿年份過濾，不再列出全部政權**（使用者提出的
> 規模疑慮：這個專案終局目標是世界史規模，全部政權攤開在一個原生 `<select>` 裡，政權
> 數一多就是不可用的落落長清單，而且用還沒存在/已滅亡的政權視角看目前顯示的年份，本身
> 也沒有敘事意義）。改用 `GET /api/v1/regimes?year=`（task 2.4 既有端點，跟 task 2.6
> 疆域查詢同一套「當年有效」判斷，不用新增後端程式碼），訂閱 `TimelineState.year` 的
> debounce 節奏跟 `TimeScrubberComponent`/`MapComponent` 一致。**上面這段「列出全部
> 政權，不篩掉沒有代稱資料的」決策已不成立，改成依年份過濾**——這是兩個不同維度的
> 篩選（有沒有代稱資料 vs. 是否為當代政權），這次調整的是後者，前者的判斷（沒有代稱
> 資料時 fallback 顯示自稱）維持不變。選到的觀察者在換年份後不再是當代政權時，自動
> 清回 `null`（「全球客觀視角」），不留一個選單選項清單裡找不到、卻還顯示選取狀態的
> 殘留值。不再需要注入 `RegimeDirectoryService`（改直接讀年份過濾後回應本身就有的
> `selfName`），移除這個依賴。
>
> 已用 `ng build`（1.52MB/341KB，budget 內）、`ng test`（212/212，新增 2 條涵蓋「觀察
> 者不再是當代政權時自動清空」/「仍是當代政權時維持選取」）、真實容器
> （`docker compose up -d --build backend frontend`）+ Chrome 擴充功能實測：225 年
> 選單正確只列吳/蜀漢/魏三筆（不是全部 8 筆政權）；選「以魏視角」後把拉桿拖到 1 年
> （無種子資料的年份），選單正確自動清回「全球客觀視角」且選項清單清空（只剩這個
> 固定選項）。
...
>
> **2026-08-31 第三次調整（使用者實機測試後的兩點 UX 回饋）**：
> 1. **時間拉桿跳轉改成獨立按鈕，不再是展開的自動副作用**——使用者回報「點標題展開
>    只是想讀內容，卻同時觸發整個 App 的時間拉桿跳年份＋地圖疆域重新渲染，兩件事
>    互搶注意力」。拆開：`toggleEvent()` 現在純粹是展開/收合，新增
>    `jumpToEventYear()` 只在使用者點展開內容裡明確的「跳到此年份」按鈕時才觸發——
>    「讀內容」跟「連動地圖」變成兩個各自獨立的使用者意圖，不再綁死成同一個點擊動作
>    的副作用。
> 2. **面板放大**：寬度 280px→400px、內文字級 `--wl-font-size-xs`→`sm`、標題
>    `sm`→`lg`、事件名稱獨立升到 `base`，留白（padding/gap）跟著等比放大，不是只放大
>    外層容器卻沒調整內容排版。
>
> 已用 `ng build`（clean）、`ng test`（218/218，`regime-event-panel.spec.ts` 新增
> 1 條專門驗證「展開本身不動時間拉桿，點跳轉按鈕才會」、既有的展開測試改成斷言展開後
...
- **矩形疆域改成真實史料多邊形**（使用者問「這個到哪一部能夠處理」，2026-08-31 研究記錄）：
  ...
  **2026-08-31 第二輪：實際做了一次 pilot，結論是「魏不行、吳可行」**——先選魏（西元
  225 年）當 pilot：交叉比對維基百科「曹魏行政區劃」條目 + 百度百科「三國行政區劃」
  條目（兩份獨立來源一致）列出黃初年間魏的完整州郡清單（司隸/豫州/冀州/兗州/徐州/
  青州/并州/雍州/涼州，約 40 個郡國），逐一去查 CHGIS 那個圖層，**結果只有彭城/下邳/
  廣陵/東海/沛國 5 個郡在 189-280 年區間內有對得上的記錄，而且全部集中在徐州邊境，
  魏真正的核心地盤（司隸洛陽、冀州鄴城一帶）完全找不到資料**——CHGIS 這個圖層對三國
  時期的空間涵蓋率明顯偏向徐州邊境跟長江以南，魏的 pilot 做不出來，回報使用者後改用
  吳。

  **吳（西元 225 年）pilot 成功**：交叉比對維基百科「孫吳行政區劃」條目列出吳在 225
  年（廣州尚未從交州分出，建安郡尚未從會稽分出——都是 264/260 年之後才有的建制，
  225 年時不該出現，不是資料缺漏）的州郡清單，對到 CHGIS 12 筆命中：武陵/長沙/零陵/
  桂陽（荊州吳部分）、丹陽/會稽/吳郡/豫章/廬陵（揚州）、合浦/蒼梧/南海（交州北部）。
  用 shapely（`unary_union`）dissolve 成單一 `MultiPolygon`，過程中 2 筆來源幾何本身
  不合法（桂陽郡、廬陵郡，`is_valid=False`），用 `buffer(0)` 修復後才能合併——這個
  修補動作本身要記錄進未來的 citation evidence note，不能悄悄做掉。合併結果目視檢查
  輪廓合理（涵蓋長江以南到嶺南、東南沿海島嶼跟海岸線細節都在，中間一塊缺口對應到
  找不到資料的南郡跟尚未設置的建安郡），比現在的矩形示意資料真實得多。**已知缺口**：
  南郡（荊州核心郡之一，三國時期魏蜀吳反覆爭奪，CHGIS 這個圖層完全沒有 189-280 年
  ...
  **2026-08-31 第四輪：確認蜀漢也做不出來，且確認整個方向沒有現成 GIS 資料可用**
  ——使用者問「那蜀漢呢」，交叉比對維基百科「蜀漢行政區劃」條目列出建興三年（225
  年，益州郡剛改名建寧郡、剛分出雲南/興古二郡）的完整 19 個郡，逐一查 CHGIS，**一個
  都找不到三國時期的記錄**——少數字面對得上的（建寧/永昌/雲南）全部是元明清時期
  （1279 年後）的同名建制，跟蜀漢的建寧/永昌/雲南差了一千多年，不是同一個東西。
  跟魏一樣的結論：CHGIS v6 這個圖層的三國時期涵蓋範圍幾乎只有長江以南到嶺南（吳的
  核心地盤），魏（華北）跟蜀漢（四川/雲貴）完全是空的，不是「再找找看」能解決的
  資料集侷限。
  **決定性佐證**：使用者貼出維基百科「蜀漢」條目的三國區劃圖，圖片版權說明寫
  「GIS 數據：ETOPO1（地形海拔）＋ Natural Earth（現代底圖）；參考地圖：中國文化
  大學版《中國歷史地圖》＋《中國歷史地圖集》（譚其驤主編）」——ETOPO1/Natural Earth
  都跟政區疆域無關，只是畫圖用的現代底稿；真正決定魏/蜀漢/吳邊界線畫在哪裡的兩本
  「參考地圖」都是**紙本出版的歷史地圖集，不是可下載的 GIS 向量資料**。連維基百科
  自己畫這張三國區劃圖，都是貢獻者對照紙本地圖集手動描邊界，不是匯出現成資料庫。
  這證實了「魏/蜀漢目前沒有可下載的 GIS 資料」不是找得不夠仔細，是這個資料領域本身
  的現狀——之後要做的話，路徑會跟維基百科這張圖一樣是照歷史地圖集手動描邊界（精度
  遠不如 CHGIS 那種真實郡級資料，本質是「模仿維基百科的畫法」），是完全不同量級
  的工作，不是查資料庫能解決的。使用者當時確認先到此為止，但後續同一天又追問了
  兩個管道，見下方第五輪。

  **2026-08-31 第五輪：追查中研院 CCTS 跟 OpenHistoricalMap，確認三個管道都碰壁**
  ——使用者問「有沒有類似但可商用的」，並解釋了一個關鍵的學界慣例：古代沒有現代
  「國界線」概念，朝廷統治力是靠設立郡縣體現的，所以譚其驤《中國歷史地圖集》跟
  CHGIS 原始考據都是以州/郡/縣為基本測繪單位，不是直接畫政權的巨大輪廓——這解釋了
  為什麼 CHGIS 對吳（郡級資料豐富）查得到、但魏蜀漢查不到不是查詢方式的問題，是
  底層測繪本身有沒有做到那個地區的問題。
  1. **CCTS（中央研究院「中華文明之時空基礎架構」）**：底層空間圖資明講「以譚其驤
     《中國歷史地圖集》為主要基礎」，涵蓋「上古至清代」全中國——如果能用，地理
     涵蓋範圍會包含魏蜀漢，不會像 CHGIS 那樣只有江南。但讀了官方 PDF「GIS資料
     最終使用者授權合約」全文（不是網頁摘要，是直接讀 PDF 原文）後確認**授權條件
     卡死了這個 app 的用法**：第 1.3 條「加值產品」僅供申請人自己使用；第 5 條
     加值產品要分送出去只能用「無法修改的格式（JPEG、GIF、BitMap 等）」——這個
     app 的 `GET /api/v1/territories` 本質就是把 GeoJSON（可修改的向量格式）即時
     分送給每個訪客，正好踩線；第 8 條還有互惠條款，加值成果要反過來授權中研院
     整合回 CCTS。且申請本身是要簽名、填單位/代表人的正式合約，不是點一下同意的
     網頁條款。**沒有替使用者申請或簽署**——這是使用者自己（或所屬單位）要決定
     簽不簽的事。
  2. **OpenHistoricalMap（PRD 已拍板的主要來源，CC0，授權完全沒問題）**：直接用
     Overpass API 查即時資料庫（不是查文件說明），(a) 全庫搜尋「魏/蜀/吳/漢」中文
     關鍵字只有 3 筆對上，全部不是三國時期（西漢紀年段、西周古蜀國）；(b) 限定
     中國周邊經緯度 + 西元 180-289 年時間窗查全部行政邊界（不限名稱），只有 2 筆：
     林邑（現在越南中部）、帶方郡（朝鮮半島漢魏邊郡），魏蜀吳核心地盤一筆都沒有。
     **確認是資料本身完全空白，不是授權問題**——OHM 是即時社群編輯的資料庫，這代表
     目前真的沒有人畫過三國中國核心地區的政區邊界，不是搜尋方式找不到。
  **三個管道总结**：CHGIS（學術非商用，只有吳的地盤）、CCTS（涵蓋全中國，但授權
  格式限制擋死互動地圖用途）、OHM（CC0 商用無虞，但資料完全空白）——目前沒有一個
  管道同時滿足「涵蓋魏蜀核心地盤」+「授權允許做成互動向量地圖」。使用者當時確認
  先到此為止，但後續同一天又追問「有沒有類似但可商用的」，見下方第六輪——最後
  嘗試過一次手動描圖路線，查證後發現來源不可信，已收回。

  **2026-08-31 第六輪：嘗試 Wikimedia Commons 插圖手動描圖，查證來源後收回**——
  使用者要求繼續找資料源。找到《三國時期行政區劃圖.svg》（Wikimedia Commons，
  公共領域，作者 Jason22/玖巧仔），藍/綠/紅分別是魏/蜀漢/吳。**技術上真的做出來
  了**：抓出 SVG 裡藍色（魏）、綠色（蜀漢）路徑，座標轉換用「這張圖裡吳的形狀
  bbox 對比已驗證過的真實吳疆域 bbox」反推仿射變換（不是肉眼點地標亂猜），套用
  後視覺上跟三國傳統地圖輪廓高度吻合，also 用 Douglas-Peucker 簡化過，程式碼
  （`RealDataSeed.SeedWeiAndShuAsync`）一度寫到能正確跑通、匯入資料庫。
  **使用者質疑「這張圖其實是路人弄出來的？」，查證後發現比預期更嚴重**：不只是
  「非專業人士畫的」，這張 SVG 跟它的前身 PNG 的「來源」欄位都只寫「自製」，**完全
  沒有標注繪製時依據了哪個地圖集或史料**——不像使用者更早貼過的另一張維基百科圖
  （三國區劃圖，那張圖明確寫參考譚其驤《中國歷史地圖集》），這張圖是徹底的黑盒子，
  可信度落在「有依據可查」跟「純粹猜測」的模糊地帶，逼近
  `docs/data-governance.md`「無法定位的二手轉述，不能單獨作為正式資料依據」那條線。
  **使用者確認不採用，已收回**：刪除 `Data/RealData/wei-225-territory.geojson`／
  `shu-225-territory.geojson`，`RealDataSeed.cs` 移除 `SeedWeiAndShuAsync`，只保留
  吳（CHGIS）那筆。**過程中意外抓到一個真的 bug**：重構時漏寫一行
  `db.Sources.Add(source)`，會讓 `SeedWuAsync` 在全新資料庫上直接 FK 違反炸掉——
  已修正，用完全重建的資料庫（新 volume）重新驗證過，`npx playwright test`（7/7）
  全數通過。
  **後續評估中研院 CCTS 是否可行**：使用者問能不能直接用 `gis.rchss.sinica.edu.tw/
  ccts/`——查證後這只是同一個 CCTS 平台的入口頁，不是不同管道，授權限制跟第五輪
  查過的完全一樣。額外讀了「機構使用授權合約」全文：這份是給大學這種機構整批簽約
  用的（教職員/學生透過所屬機構取得使用權），**個人不是這份合約的申請對象**，個人
  要用的話要走另一份「最終使用者授權合約」（見第五輪已讀過的全文）。**一個值得
  之後重新檢視的細節，這裡先記下來，不預先下結論**：該合約第 1.3 條「加值產品…
  以專供乙方使用為限」的限制對象是「分送」，如果這個 app 現階段真的只有使用者
  自己使用（符合憲法目前「單人自用」的階段），理論上可能不觸發第 5 條的格式限制
  ——但這是需要使用者自己判斷、甚至直接聯繫中研院確認的法律解讀，不是這裡能自行
  拍板的事；且仍未確認 CCTS 系統本身是否真的提供向量資料下載（shapefile/GeoJSON）
  還是只有網頁瀏覽，這點也需要使用者實際申請/詢問才能確認。**現況**：魏跟蜀漢
  維持矩形示意，CCTS 是否可行留給使用者自行決定要不要申請帳號、聯繫中研院確認。
