...
>
> **2026-08-29 更新**：憲法新增 R4（中英雙語內容支援），新增 2.16（雙語 schema 第一層）與 2.17（既有 seed 資料英文翻譯內容），並回頭在 2.4/2.8/2.10/2.12/2.13 的任務描述補上「依賴 2.16、支援 `?locale=`」——這幾個端點都還沒動工，直接在任務描述裡註明比事後才發現要補快。2.3（`reign_eras`）已完成且排在雙語第二層，不回頭改。詳細範圍分層與已知風險（`sections` JSONB 翻譯結構未拍板）見 PRD §6「多語言內容設計」。
...
| [ ] | 2.4 | 政權查詢端點（唯讀） | `GET /api/v1/regimes`（支援 `?year=`/`?period=` 過濾）、`GET /api/v1/regimes/:id`。**依賴 2.16**：`regimes` 是雙語第一層，查詢要支援 `?locale=`（省略時回原始語言，有翻譯列時回翻譯內容，見 PRD §6「多語言內容設計」） | §7 | 1 個 |
...
| [ ] | 2.8 | 史觀主線 preset 查詢端點 | `GET /api/v1/lineage-presets`、`GET /api/v1/lineage-presets/:id/regimes`。**依賴 2.16**：`lineage_presets` 是雙語第一層，同 2.4 支援 `?locale=` | §7 | 1 個 |
...
| [ ] | 2.10 | 事件骨幹 CRUD | `GET /api/v1/events?year={y}`、`GET /api/v1/events/:id`、`POST /api/v1/events`（寫入時呼叫 2.2 EdtfService，含 `parent_event_id` 組成關係）。**2.2 的 `EdtfService.TryParse` 只驗證單一字串合法性，不驗證跨欄位邏輯**——這裡要另外補一條檢查：換算出的 `end_decimal` 不可早於 `start_decimal`（避免使用者填反開始/結束時間），憲法/PRD 沒有明講這條但屬於基本資料完整性，不需要另外拍板。**依賴 2.16**：`historical_events` 是雙語第一層（`name` 欄位），同 2.4 支援 `?locale=`；`sections` JSONB 內嵌文字的翻譯結構尚未拍板，見 PRD §6「已知風險」，這裡動工前要先決定 | §7 | 1 個 |
| [ ] | 2.11 | 事件類型標籤 | `GET /api/v1/event-tags`（列出可用標籤）、事件寫入端點（2.10）支援帶 `tag_ids` 陣列建立 `historical_event_tag_map` | §6 事件三維度 | 1 個 |
| [ ] | 2.12 | 觀察者類別 + 多重視角敘事 | **寫入端點實作前先定義 `primary_sources`/`claimed_casualties` 的 JSON schema 與最小 citation 欄位（§12 TODO）**；`GET /api/v1/observer-categories`、`GET /api/v1/events/:id/perspectives`、`POST .../perspectives`（應用層驗證 `regime_id`/`observer_category_id` 至少擇一非 NULL）。**依賴 2.16**：`historical_event_perspectives` 是雙語第一層，同 2.4 支援 `?locale=` | §6、Story 3、§12 TODO | 1 個 |
| [ ] | 2.13 | 事件爭議點 | **寫入端點實作前先定義 `viewpoints` 的 JSON schema 與最小 citation 欄位（§12 TODO，可與 2.12 併案決定）**；`GET /api/v1/events/:id/controversies`、`POST .../controversies`。**依賴 2.16**：`historical_event_controversies` 是雙語第一層，同 2.4 支援 `?locale=` | §6 notes §十.2、§12 TODO | 1 個 |
| [ ] | 2.16 | 雙語內容第一層 schema（憲法 R4） | 新增 5 張 `_translations` companion 表（`regime_translations`／`historical_event_translations`／`historical_event_perspective_translations`／`historical_event_controversy_translations`／`lineage_preset_translations`，見 PRD §6「多語言內容設計」）：EF Core entities/configurations + migration。`historical_event_translations` 的 `sections` JSONB 翻譯結構要先拍板再動工（PRD §6「已知風險」）。純新增表，不影響現有資料 | §6、憲法 R4 | 1 個（或依 5 張表拆多個 commit） |
| [ ] | 2.17 | 既有 seed 資料英文翻譯內容 | 幫 2.16 的 5 張表補上既有種子資料（漢/魏/蜀漢/吳/晉、赤壁/漢禪魏/魏禪晉、兩個 lineage preset 等）對應的英文內容。**這是內容撰寫工作，不是純工程工作**——翻譯品質需要人工核對，不是機械轉換；可以分批進行，不需要一次補齊全部才能讓 schema/API 上線（沒有翻譯的內容 fallback 回中文，見 PRD §6） | §6、憲法 R4 | 依內容量拆多個 commit |
