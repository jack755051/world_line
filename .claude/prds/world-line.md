...
| UI 元件庫 | Sanring UI（`@sanring/cli`）+ Tailwind CSS v4 | 毛玻璃側邊抽屜／手風琴／多重視角分頁等 headless 元件（notes §十一原開放問題） | **已完成（2026-08-29，task 3.0）**：source-first、非傳統 npm 依賴——CLI 把元件原始碼複製進 `app/src/app/components/ui`，團隊自行維護；Tailwind v4 用官方 `@tailwindcss/postcss` PostCSS 外掛安裝（`app/.postcssrc.json`）。**`src/sanring-theme.css` 語意層（`--sanring-background/-foreground/-border/-surface/-control/-primary` 等，所有元件實際引用的都是這層）已全部改成 alias 到 `design-tokens.scss` 的 `--wl-*`**，不是併存兩套色盤，同時移除了 CLI 預設產生的深/淺色雙軌設計（改成單一淺色語意層，見下方「政權識別色…」段落旁的系統色說明所依循的同一個「❌ 不做深色模式」原則）。已用 Button 元件驗證：`ng build`/`ng test`（12/12）通過、編譯後 CSS 內 `focus-visible`/`hover` 規則正確指到 alias 過的變數。**刻意未處理、留待有元件實際用到再補**：Sanring 原廠的 `--sanring-primary-10..90` 品牌色階跟 `coral/sun/info/success/warn/error` 色相家族，還沒對齊 `--wl-*`（例：Button 的 destructive variant 目前吃 Sanring 原廠紅色，不是 `--wl-status-critical`），見 §12 開放問題 |
...
- [ ] TODO：任務 3.0 刻意留下的缺口——`src/sanring-theme.css` 的 `--sanring-primary-10..90`、`coral/sun/info/success/warn/error` 色相家族還是 Sanring 原廠預設值，沒對齊 `--wl-*`。等真的裝進會用到這些家族色的元件（badge、alert、destructive 按鈕確定要上線）時，決定是要整組重新產生對到 `--wl-status-*`/主色曲線，還是保留 Sanring 原廠配色當「元件庫自己的次要語意色」。
