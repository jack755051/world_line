...
- [ ] Given 聚焦政權與周邊政權之間存在已建檔的離散互動事件（如戰爭、條約），when 顯示互動清單，then 應列出可點擊追溯至對應 `historical_events` 記錄的入口；若是持續性關係（如貿易、朝貢），則列出對應 `regime_relations` 記錄的入口（已拍板，見 §6）——**未做**：依賴的後端端點（task 2.9 政權關係、2.10 事件骨幹）都還沒實作，沒有資料可以連結，見 implementation plan 任務 3.7 的說明
